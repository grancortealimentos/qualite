{{-- resources/views/livewire/propriedades/index.blade.php --}}
    ...
    <div class="bg-surface border border-border rounded-xl overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center border-b border-border">
            <div class="flex items-center gap-x-2">
                ...
                <h2 class="text-lg font-semibold text-ink">{{ __('Lista de Propriedades') }}</h2>
            </div>

            @can('propriedades.criar')
                <a href="{{ route('propriedades.create') }}" wire:navigate
            ...
            <div class="p-1.5 min-w-full inline-block align-middle">
                <div class="bg-surface border border-border rounded-xl overflow-hidden relative">
                        ...
                        <thead>
                            <tr>
                                <th scope="col" class="px-6 py-3 text-start">
                                    <span
                                        class="text-xs font-semibold uppercase text-primary-light">{{ __('Produtor') }}</span>
                                </th>
                                <th scope="col" class="px-6 py-3 text-start">
                                    <span
                                        class="text-xs font-semibold uppercase text-primary-light">{{ __('Propriedade') }}</span>
                                </th>
                                <th scope="col" class="px-6 py-3 text-start">
                        ...
                        <tbody class="divide-y divide-border">
                            @forelse ($propriedades as $propriedade)
                                @php
                                    ...
                                    $avatarStyle = $avatarStyles[$propriedade->id % count($avatarStyles)];

                                    // Iniciais ignorando dígitos: "10101-GRAN CORTE" => "GR"
                                    $iniciais = \Illuminate\Support\Str::of($propriedade->razao_social)
                                        ->replaceMatches('/[^a-zA-Z]/', '')
                                        ->substr(0, 2)
                                        ->upper();
                                @endphp
                                <tr wire:key="propriedade-{{ $propriedade->id }}"
                                    class="hover:bg-surface-hover {{ $propriedade->eh_ativo ? '' : 'opacity-60' }}">

                                    <td class="size-px whitespace-nowrap">
                                        <div class="px-6 py-3">
                                            <span class="block text-sm text-ink truncate">
                                                {{ $propriedade->produtor?->nome_completo ?? __('—') }}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="h-px w-72 min-w-72">
                                        <div class="px-6 py-3 flex items-center gap-x-3">
                                            ...
                                            <div class="min-w-0">
                                                <span
                                                    class="block text-sm font-medium text-ink truncate">{{ $propriedade->razao_social }}</span>
                                                <span class="block text-xs text-ink-muted truncate">
                                                    {{ $propriedade->nome_fantasia ?: __('Sem nome fantasia') }}
                                                </span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="size-px whitespace-nowrap">
                                        <div class="px-6 py-3">
                                            @if ($propriedade->cnpj)
                                                <span
                                                    class="block text-sm text-ink">{{ $propriedade->cnpj_formatado }}</span>
                                                @if ($propriedade->ie)
                                    ...
                                    <td class="size-px whitespace-nowrap">
                                        <div class="px-6 py-3">
                                            <span
                                                class="inline-flex items-center gap-1.5 py-1 px-2 rounded-full text-xs font-medium {{ $propriedade->eh_ativo ? 'bg-success/15 text-success' : 'bg-danger/15 text-danger' }}">
                                                <span
                                                    class="size-1.5 inline-block rounded-full {{ $propriedade->eh_ativo ? 'bg-success' : 'bg-danger' }}"></span>
                                                {{ $propriedade->eh_ativo ? __('Ativa') : __('Inativa') }}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="size-px whitespace-nowrap">
                                        <div class="px-6 py-3 flex justify-end items-center gap-x-1.5">
                                            @can('propriedades.editar')
                                                <a href="{{ route('propriedades.edit', $propriedade) }}" wire:navigate
                                            ...
                                            @can('propriedades.alterar_status')
                                                <button type="button" wire:click="alternarStatus({{ $propriedade->id }})"
                                                    ...
                                                    title="{{ $propriedade->eh_ativo ? __('Desativar') : __('Ativar') }}">
                                                    <svg class="shrink-0 size-3.5" xmlns="http://www.w3.org/2000/svg"
                                                        width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                        stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                        stroke-linejoin="round">
                                            ...
                                            @can('propriedades.excluir')
                                                <button type="button" wire:click="excluir({{ $propriedade->id }})"
                                                    wire:confirm="{{ __('Tem certeza que deseja excluir esta propriedade?') }}"
                                                    wire:loading.attr="disabled"
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        ...
                                        <p class="text-sm text-ink-muted">
                                            @if ($this->temQualquerFiltro())
                                                {{ __('Nenhuma propriedade encontrada com esses filtros.') }}
                                            @else
                                                {{ __('Nenhuma propriedade cadastrada ainda.') }}
                                            @endif
                                        </p>
                                    </td>
                                </tr>
                    ...
                    <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-border">
                        <div>
                            <p class="text-sm text-ink-muted">
                                <span class="font-semibold text-ink">{{ $propriedades->total() }}</span>
                                {{ trans_choice('propriedade encontrada|propriedades encontradas', $propriedades->total()) }}
                            </p>
                        </div>
                        <div>
                            {{ $propriedades->links() }}
